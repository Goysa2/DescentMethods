...
This version of the globalized Shamanskii has no criterion to determine when
to update or not to update the hessian matrix. It simply computes it every m
iteration (an attribute that can be changed through the mem parameter).
Inputs:
    ...
    - mem, the number of iteration an hessian matrix will be reused.
Outputs:
